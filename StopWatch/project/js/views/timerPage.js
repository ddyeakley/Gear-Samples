...
define({
    ...
    def: function viewsTimerPage(req) {
        ...
        function timeAlarmFire(event) {
            ...
            // Condition below handling situation when
            // timer countdown is started, application is closed
            // and the power button is pressed at the moment
            // when alarm has to invoke
            if (pageHelper.isPageActive(timerUpPage)) {
                reset();
                app.exit();
            }
        ...
        function visibilityChange(ev) {
            ...
            if (state !== 'visible') {
                ...
                // Situations handling by conditions below:
                // 1. application is hidden or closed
                //    alarm Page is active, on power button pressed
                // 2. page 'timer' is active, countdown on, 1 sec. left
                //    in middle of 1 left second power button is pressed
                if ((pageHelper.isPageActive(timerUpPage) && powerKeyDown) ||
                        (!alarmInvoked && alarmStatus)
                        ) {
                    reset();
                    app.exit();
                }
        ...
        e.listeners({
            ...
            'app.timeAlarm': timeAlarmFire,
            'views.initPage.visibility.change': visibilityChange,
            'views.initPage.device.powerOff': onPowerOff,
            'core.storage.idb.open': init,
            'core.storage.idb.read': onStorageGet,
            'core.storage.idb.write': onStorageWrite,
            'views.stopWatchPage.changeActivePage': onChangeActivePage
        });
